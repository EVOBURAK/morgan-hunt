-- =================================================================================
-- 🌿 MORGAN HUB V5.0 (ADVANCED BLOX FRUITS EDITION) 🌿
-- =================================================================================
...
Title.Text = "🌿 MORGAN HUB V5.0 (BLOX FRUITS)"
...
local function addDropdown(text, options, default, callback)
    ...
    btn.Position = UDim2.new(0.56, 0, 0.15, 0)
...
addToggle("📜 Auto Get Quest", Settings.AutoQuest, function(v) Settings.AutoQuest = v end)
addToggle("⚔️ Auto M1 / Fast Attack", Settings.EnableM1, function(v) Settings.EnableM1 = v end)

addDropdown("🗡️ Weapon Type", {"Melee", "Sword", "Gun", "Blox Fruit"}, Settings.WeaponType, function(v)
    Settings.WeaponType = v
end)

addToggle("📊 Auto Stats", Settings.AutoStats, function(v) Settings.AutoStats = v end)
addDropdown("📊 Stat Choice 1", {"Melee", "Defense", "Sword", "Gun", "Blox Fruit"}, Settings.SelectedStats.Stat1, function(v) Settings.SelectedStats.Stat1 = v end)
addDropdown("📊 Stat Choice 2", {"Melee", "Defense", "Sword", "Gun", "Blox Fruit"}, Settings.SelectedStats.Stat2, function(v) Settings.SelectedStats.Stat2 = v end)
addDropdown("📊 Stat Choice 3", {"Melee", "Defense", "Sword", "Gun", "Blox Fruit"}, Settings.SelectedStats.Stat3, function(v) Settings.SelectedStats.Stat3 = v end)
...
-- =============================================================
-- AUTO QUEST & STATS & WEAPON ENGINE
-- =============================================================
...
local function checkAndGetQuest()
    ...
    pcall(function()
        ...
        if not questFrame or not questFrame.Visible then
            local qName, qLevel, _ = getQuestData()
...
local function equipSelectedWeapon()
    ...
    local targetType = toolTypeMap[Settings.WeaponType]
    
    -- Elindeyse ve tipi eşleşiyorsa işlem yapma
    local currentTool = char:FindFirstChildOfClass("Tool")
    if currentTool and currentTool:FindFirstChild("ToolTip") and currentTool.ToolTip == targetType then
        return
    end
    
    -- Sırt çantasından bul ve tak
    for _, tool in pairs(LocalPlayer.Backpack:GetChildren()) do
        if tool:IsA("Tool") and tool:FindFirstChild("ToolTip") and tool.ToolTip == targetType then
    ...
    while task.wait(0.5) do
        if Settings.AutoStats then
            pcall(function()
                ...
                if points > 0 then
                    local statsToBoost = {
                        Settings.SelectedStats.Stat1,
                        Settings.SelectedStats.Stat2,
                        Settings.SelectedStats.Stat3
                    }
                    for _, statName in ipairs(statsToBoost) do
...
task.spawn(function()
    while task.wait(0.1) do
        if Settings.EnableM1 and (Settings.AutoFarm or Settings.AutoHunt) then
            pcall(function()
...
game.StarterGui:SetCore("SendNotification", {
    ...
    Text = "Blox Fruits Modules Loaded Successfully!",
    Duration = 4
})
