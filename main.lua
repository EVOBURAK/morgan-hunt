...
local U, UI, Move, Combat, Farm, Items, PvP, ESP, Misc, Data =
    {Conns = {}}, {T = {}}, {}, {}, {}, {}, {}, {}, {}, {}
...
local Cfg = {
    ...
    FarmHeight = 18, TweenSpeed = 280,
    BringMobs = true, BringRadius = 380, Hitbox = true, HitboxSize = 45,
    FastAttack = true, AttackMode = "Ultra (Net+Combat)", AttackRange = 70,
    AutoBuso = true, AutoKen = false,
    TargetLevel = 2800, RaidAfterMax = true, AutoSeaTravel = true,
    FarmNearest = false, FarmMob = false, SelectedMob = "",
    FarmBoss = false, SelectedBoss = "",
    PirateRaid = false, EliteHunter = false,
...
function UI.CreateWindow(titleText, subtitleText)
    ...
    local Rain = New("Frame", {
        Name = "Rain", Size = UDim2.new(1, 0, 1, 0), BackgroundTransparency = 1,
        ClipsDescendants = true, ZIndex = 1,
    }, Main)
    local drops = {}
    local DROPS = (W < 520) and 32 or 58
    for i = 1, DROPS do
        local f = New("Frame", {
            Size = UDim2.new(0, 1, 0, math.random(10, 24)),
            BackgroundColor3 = Color3.fromRGB(175, 205, 255),
            BackgroundTransparency = 0.5 + math.random() * 0.3,
            BorderSizePixel = 0, Rotation = 12, ZIndex = 1,
        }, Rain)
        drops[i] = {f = f, x = math.random(0, W), y = math.random(-H, H), s = math.random(420, 820)}
    end
    UI.RainFrame = Rain
    U.Conn(RunService.RenderStepped, function(dt)
        Rain.Visible = Cfg.Rain and Main.Visible
        if not Rain.Visible then return end
        for _, d in ipairs(drops) do
            d.y = d.y + d.s * dt
            d.x = d.x - d.s * 0.2 * dt
            if d.y > H + 20 or d.x < -20 then
                d.y = -30
                d.x = math.random(0, W + 60)
            end
    ...
    New("TextLabel", {
        Size = UDim2.new(1, -170, 0, 24), Position = UDim2.new(0, 52, 0, 4), BackgroundTransparency = 1,
    ...
    New("TextLabel", {
        Size = UDim2.new(1, -170, 0, 14), Position = UDim2.new(0, 52, 0, 27), BackgroundTransparency = 1,
    ...
    MinBtn.MouseButton1Click:Connect(function() Main.Visible = false; Float.Visible = true end)
    ...
    function Window:CreateTab(name, icon)
        ...
        function Tab:Button(text, callback)
            local b = New("TextButton", {
                Size = UDim2.new(1, 0, 0, 36), BackgroundColor3 = Theme.Accent, BackgroundTransparency = 0.4,
...
task.spawn(function()
    pcall(function()
        ...
        if cfm and Ex.GetUpvalues then
            Combat.Framework = Ex.GetUpvalues(require(cfm))[2]
        end
    end)
end)
...
U.Loop(0.6, function()
    ...
    if Cfg.SkillZ then Combat.PressKey(Enum.KeyCode.Z) end
    if Cfg.SkillX then Combat.PressKey(Enum.KeyCode.X) end
    if Cfg.SkillC then Combat.PressKey(Enum.KeyCode.C) end
    if Cfg.SkillV then Combat.PressKey(Enum.KeyCode.V) end
...
function Farm.EquipSelected()
    ...
    local tool, equipped = Farm.GetTool(Cfg.WeaponType)
...
-- Best-effort: use the submarine to reach Submerged Island (needs the Tyrant of the Skies boss beaten).
function Farm.EnterSubmerged()
    if os.clock() - Farm.LastSubmerged < 20 then return end
    Farm.LastSubmerged = os.clock()
    local pos = Farm.FindNPC("submarine worker")
    if not pos then
...
function Farm.Fight(name, mobPos, row)
    ...
    if not (target and target.Parent and U.MobAlive(target) and U.CleanName(target.Name):lower() == name:lower()) then
    ...
    if pos then
        ...
        Move.To(CF(pos + V3(0, Cfg.FarmHeight, 0)))
    end
end
...
function Farm.Level()
    ...
    local lvl = U.Level()
    if lvl >= Cfg.TargetLevel then
        Farm.Status("Target level " .. Cfg.TargetLevel .. " reached")
        if UI.T.AutoFarm then UI.T.AutoFarm.Set(false) else Cfg.AutoFarm = false end
        if Cfg.RaidAfterMax and UI.T.PirateRaid then UI.T.PirateRaid.Set(true) end
        U.Notify("Auto Farm", "Target level reached. " .. (Cfg.RaidAfterMax and "Switching to Pirate Raid." or ""), 6)
        return
    end
    ...
    -- far-away dynamic quests (Submerged Island)
    if row[7]:sub(1, 7) == "Dynamic" and (root.Position - row[6]).Magnitude > 6000 then
        Farm.Status("Traveling to Submerged Island")
        Farm.EnterSubmerged()
        return
    end
    ...
    if Cfg.UseQuest then
        ...
        if q and q.Visible then
            if not Farm.TitleMatches(Farm.QuestTitle(), row) then
...
function Farm.NearestStep()
    ...
    Farm.Prep()
    Move.To(best.HumanoidRootPart.CFrame * CF(0, Cfg.FarmHeight, 0) * CFrame.Angles(math.rad(-90), 0, 0))
...
function Farm.FightBoss(name)
    ...
    if target then
        Farm.Prep()
        Move.To(target.HumanoidRootPart.CFrame * CF(0, math.max(Cfg.FarmHeight, 22), 0) * CFrame.Angles(math.rad(-90), 0, 0))
        Combat.Request()
...
function Farm.PirateRaidStep()
    ...
    if Cfg.AutoFarm or Cfg.FarmBoss or Cfg.FarmMob or Cfg.FarmNearest then return false end
    Farm.Status("Pirate Raid: waiting at Castle on the Sea")
    if (root.Position - Data.CastlePos).Magnitude > 600 then
        Move.To(CF(Data.CastlePos + V3(0, 60, 0)), 350)
    end
    return true
end
...
function Items.CollectStep()
    ...
    if bd < 12 then
        ...
        Move.To(best.CFrame * CF(0, 2, 0), 350)
    end
    return true
end
...
function Misc.FpsBoost()
    ...
    UI.Notify("FPS Boost", "Textures and effects reduced.", 4)
end
...
function Misc.Hop()
    ...
    UI.Notify("Server Hop", "No suitable server found.", 4)
end

-- client-side fake fruit (only you can see it)
function Misc.SpawnFakeFruit(fruitName, color)
    local root = U.Root()
    if not root then return end
    local p = Instance.new("Part")
    p.Name = fruitName
    p.Size = V3(1.2, 1.2, 1.2)
...
function Misc.Debug()
    ...
    add("Quest title: " .. Farm.QuestTitle())
    add("CombatRemotes: attack=" .. tostring(Combat.RegAttack ~= nil) .. " hit=" .. tostring(Combat.RegHit ~= nil)
        .. " framework=" .. tostring(Combat.Framework ~= nil))
    pcall(function()
        ...
        for qName, qTable in pairs(Quests) do
            for idx, info in pairs(qTable) do
                if type(info) == "table" and type(info.LevelReq) == "number" and info.LevelReq >= 2500 then
                    local mob
                    for k in pairs(info.Task or {}) do mob = k break end
                    add(string.format("QUEST %s #%s req=%d mob=%s", tostring(qName), tostring(idx), info.LevelReq, tostring(mob)))
                end
            end
        end
    end)
    ...
    if npcs then
        for _, n in ipairs(npcs:GetChildren()) do
            ...
            if ln:find("submerged") or ln:find("submarine") then
...
Cfg.SelectedMob = Data.MobList[1] or ""
Cfg.SelectedBoss = (Data.Bosses[Sea] and Data.Bosses[Sea][1]) or ""

-- ---------------------------------------------------------------------------
-- HOME
-- ---------------------------------------------------------------------------
TabHome:Section("Status")
local LblStatus = TabHome:Label("Status: Idle")
local LblInfo = TabHome:Label("Loading...")
TabHome:Section("Quick Actions")
TabHome:Button("Stop Everything", function()
    for _, k in ipairs({"AutoFarm", "PirateRaid", "AutoChest", "AutoCollectFruit"}) do
        if UI.T[k] then UI.T[k].Set(false) end
    end
    for _, k in ipairs({"FarmNearest", "FarmMob", "FarmBoss", "EliteHunter", "PvpKill", "PvpHunt"}) do
        if UI.T[k] then UI.T[k].Set(false) else Cfg[k] = false end
    end
    Move.StopTeleport()
    U.Notify("Morgan Hub", "All automation stopped.", 3)
end)
TabHome:Section("About")
TabHome:Label("Morgan Hub V3. Use the tabs on the left. Scroll every page with your finger or mouse wheel. "
    .. "Use the '-' button to hide the window and the round 'M' button to bring it back.")
...
U.Loop(0.5, function()
    ...
    LblInfo.Set(string.format("Level: %d  |  Sea: %d  |  FPS: %d\nTarget: %s\nWeapon: %s",
        U.Level(), Sea, math.floor(fpsSmooth),
        row and (row[2] .. " (" .. row[7] .. ")") or "-", Cfg.WeaponType))
end)
...
TabFarm:Toggle("Auto Farm Level (Quest + Island by Level)", false, function(v)
    Cfg.AutoFarm = v
    if not v then Move.Cancel() Farm.Target = nil end
end, "AutoFarm")
TabFarm:Toggle("Take Quests", true, function(v) Cfg.UseQuest = v end)
TabFarm:Dropdown("Weapon Type", {"Melee", "Sword", "Gun", "Blox Fruit"}, "Melee", function(v) Cfg.WeaponType = v end)
TabFarm:Toggle("Auto Buso Haki", true, function(v) Cfg.AutoBuso = v end)
TabFarm:Toggle("Auto Observation (Ken)", false, function(v) Cfg.AutoKen = v end)
TabFarm:Slider("Target Level", 100, 3000, 2800, function(v) Cfg.TargetLevel = v end, 25)
TabFarm:Toggle("Start Pirate Raid At Target Level", true, function(v) Cfg.RaidAfterMax = v end)
TabFarm:Toggle("Auto Travel To Next Sea", true, function(v) Cfg.AutoSeaTravel = v end)

TabFarm:Section("Combat")
TabFarm:Toggle("Fast Attack", true, function(v) Cfg.FastAttack = v end)
TabFarm:Dropdown("Attack Mode", {"Ultra (Net+Combat)", "Combat Framework", "Click Only"}, "Ultra (Net+Combat)",
    function(v) Cfg.AttackMode = v end)
TabFarm:Slider("Attack Range", 30, 120, 70, function(v) Cfg.AttackRange = v end, 5)
TabFarm:Toggle("Magnet (Bring Mobs)", true, function(v) Cfg.BringMobs = v end)
TabFarm:Slider("Magnet Radius", 100, 600, 380, function(v) Cfg.BringRadius = v end, 20)
TabFarm:Toggle("Hitbox Expander (Mobs)", true, function(v) Cfg.Hitbox = v end)
TabFarm:Slider("Hitbox Size", 10, 80, 45, function(v) Cfg.HitboxSize = v end, 5)
TabFarm:Slider("Farm Height", 5, 40, 18, function(v) Cfg.FarmHeight = v end, 1)
TabFarm:Slider("Tween Speed", 100, 400, 280, function(v) Cfg.TweenSpeed = v end, 10)

TabFarm:Section("Auto Skills (only while fighting)")
TabFarm:Toggle("Skill Z", false, function(v) Cfg.SkillZ = v end)
TabFarm:Toggle("Skill X", false, function(v) Cfg.SkillX = v end)
TabFarm:Toggle("Skill C", false, function(v) Cfg.SkillC = v end)
TabFarm:Toggle("Skill V", false, function(v) Cfg.SkillV = v end)
TabFarm:Toggle("Skill F", false, function(v) Cfg.SkillF = v end)

-- ---------------------------------------------------------------------------
-- MORE FARM
-- ---------------------------------------------------------------------------
TabMore:Section("Mob Farming")
TabMore:Toggle("Farm Nearest Mob (Mob Aura)", false, function(v)
    Cfg.FarmNearest = v
    if not v then Move.Cancel() end
end, "FarmNearest")
TabMore:Dropdown("Select Mob", Data.MobList, Cfg.SelectedMob, function(v) Cfg.SelectedMob = v end)
TabMore:Toggle("Farm Selected Mob", false, function(v)
    Cfg.FarmMob = v
    if not v then Move.Cancel() end
end, "FarmMob")
...
TabMore:Toggle("Farm Selected Boss", false, function(v)
    Cfg.FarmBoss = v
    if not v then Move.Cancel() end
end, "FarmBoss")
TabMore:Toggle("Auto Elite Hunter", false, function(v)
    Cfg.EliteHunter = v
    if not v then Move.Cancel() end
end, "EliteHunter")

TabMore:Section("Events")
TabMore:Toggle("Auto Pirate Raid (Castle on the Sea)", false, function(v)
    Cfg.PirateRaid = v
    if not v then Move.Cancel() end
end, "PirateRaid")
TabMore:Toggle("Auto Collect Chests", false, function(v)
    Cfg.AutoChest = v
    if not v then Move.Cancel() end
end, "AutoChest")
TabMore:Label("Sword quest chains (Saber, Rengoku, CDK...) change often and are not automated. "
    .. "Use Boss Farm for sword drops and the Shop tab to buy dealer swords.")

-- ---------------------------------------------------------------------------
-- FRUITS
-- ---------------------------------------------------------------------------
TabFruit:Section("Collect / Store")
TabFruit:Toggle("Auto Collect Fruit", false, function(v)
    Cfg.AutoCollectFruit = v
    if not v then Move.Cancel() end
end, "AutoCollectFruit")
TabFruit:Toggle("Auto Store Fruit", false, function(v) Cfg.AutoStoreFruit = v end)
TabFruit:Button("Store All Fruits Now", function()
    local n = Items.StoreAll()
    UI.Notify("Fruits", "Tried to store " .. n .. " fruit(s).", 3)
end)
TabFruit:Toggle("Fruit Spawn Notifier", true, function(v) Cfg.FruitNotify = v end)
TabFruit:Toggle("Fruit ESP", false, function(v) Cfg.ESPFruit = v end)

TabFruit:Section("Random Fruit (costs Beli)")
TabFruit:Button("Buy Random Fruit Now", function() U.Comm("Cousin", "Buy") end)
TabFruit:Toggle("Auto Buy Random Fruit", false, function(v) Cfg.AutoRandomFruit = v end)
TabFruit:Label("Note: fruit rarity is rolled on the server. A client script cannot raise luck, so there is no fake "
    .. "'100x luck' toggle. Auto Buy + Auto Store just repeats the normal roll for you.")

-- ---------------------------------------------------------------------------
-- STATS
-- ---------------------------------------------------------------------------
TabStats:Section("Auto Stats")
TabStats:Toggle("Enable Auto Stats", false, function(v) Cfg.StatOn = v end)
TabStats:Toggle("Melee", false, function(v) Cfg.StatMelee = v end)
TabStats:Toggle("Defense", false, function(v) Cfg.StatDefense = v end)
TabStats:Toggle("Sword", false, function(v) Cfg.StatSword = v end)
TabStats:Toggle("Gun", false, function(v) Cfg.StatGun = v end)
TabStats:Toggle("Blox Fruit", false, function(v) Cfg.StatFruit = v end)
TabStats:Slider("Points Per Tick", 1, 99, 3, function(v) Cfg.StatAmount = v end, 1)
...
TabPvp:Toggle("Skip Teammates / Forcefield", true, function(v) Cfg.PvpSkipTeam = v end)
TabPvp:Toggle("Camera Aimbot", false, function(v) Cfg.Aimbot = v end)
TabPvp:Slider("Aimbot FOV", 60, 600, 250, function(v) Cfg.AimFOV = v end, 10)
TabPvp:Toggle("Player Hitbox Expander", false, function(v) Cfg.PlayerHitbox = v end)
TabPvp:Slider("Player Hitbox Size", 5, 50, 20, function(v) Cfg.PlayerHitboxSize = v end, 1)
TabPvp:Toggle("Player ESP", false, function(v) Cfg.ESPPlayer = v end)
...
TabPlayer:Toggle("WalkSpeed", false, function(v)
    Cfg.Speed = v
    if not v then local h = U.Hum() if h then h.WalkSpeed = 16 end end
end)
TabPlayer:Slider("WalkSpeed Value", 16, 300, 100, function(v) Cfg.SpeedVal = v end, 2)
TabPlayer:Toggle("JumpPower", false, function(v)
    Cfg.Jump = v
    if not v then local h = U.Hum() if h then h.UseJumpPower = true h.JumpPower = 50 end end
end)
TabPlayer:Slider("JumpPower Value", 50, 400, 100, function(v) Cfg.JumpVal = v end, 5)
TabPlayer:Toggle("Infinite Jump", false, function(v) Cfg.InfJump = v end)
TabPlayer:Toggle("Noclip", false, function(v) Cfg.Noclip = v end)
TabPlayer:Toggle("Fly", false, function(v) Cfg.Fly = v end)
TabPlayer:Slider("Fly Speed", 30, 300, 90, function(v) Cfg.FlySpeed = v end, 5)
TabPlayer:Toggle("Walk On Water", false, function(v) Cfg.WalkWater = v Misc.WaterWalk(v) end)
...
TabVisual:Toggle("Stretched Screen", false, function(v) Cfg.Stretch = v end)
TabVisual:Slider("Stretch Amount", 0.3, 1, 0.65, function(v) Cfg.StretchAmt = v end, 0.05)
TabVisual:Toggle("Rain Effect (GUI)", true, function(v) Cfg.Rain = v end)
TabVisual:Toggle("Fullbright", false, function(v) Cfg.Fullbright = v Misc.SetFullbright(v) end)
TabVisual:Toggle("No Fog", false, function(v) Cfg.NoFog = v Misc.SetNoFog(v) end)
TabVisual:Button("FPS Boost (one time)", function() Misc.FpsBoost() end)
TabVisual:Section("ESP")
TabVisual:Toggle("Mob ESP", false, function(v) Cfg.ESPMob = v end)
TabVisual:Toggle("Chest ESP", false, function(v) Cfg.ESPChest = v end)
TabVisual:Label("Player ESP is in the PVP tab, Fruit ESP is in the Fruits tab.")

-- ---------------------------------------------------------------------------
-- FUN
-- ---------------------------------------------------------------------------
TabFun:Section("Fake Fruits (only you can see them)")
for _, f in ipairs({
    {"Leopard Fruit", Color3.fromRGB(255, 215, 0)}, {"Dough Fruit", Color3.fromRGB(255, 255, 224)},
    {"Dragon Fruit", Color3.fromRGB(255, 100, 100)}, {"Venom Fruit", Color3.fromRGB(138, 43, 226)},
    {"Buddha Fruit", Color3.fromRGB(255, 223, 0)}, {"Kitsune Fruit", Color3.fromRGB(135, 206, 250)},
}) do
    TabFun:Button("Spawn Fake " .. f[1], function() Misc.SpawnFakeFruit(f[1], f[2]) end)
end
...
UI.Notify("Morgan Hub V3", "Loaded. Sea " .. Sea .. " | Level " .. U.Level(), 5)
print("[MorganHub] V3.0 loaded. Sea " .. Sea)
