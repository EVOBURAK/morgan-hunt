...
function UI.CreateWindow(titleText, subtitleText)
    ...
    U.Conn(RunService.RenderStepped, function(dt)
        ...
        if os.clock() >= nextFlash then
            nextFlash = os.clock() + math.random(8, 18)
...
U.Loop(0.05, function()
    ...
    -- low HP escape: fly high up until health recovers
    if Cfg.LowHPEscape then
        ...
        if Farm.Escaping then
            if frac >= 0.85 then
                ...
                Move.To(CF(root.Position.X, Farm.EscapeY, root.Position.Z), 400)
                return
            end
        end
    end
...
TabHome:Button("Stop Everything", function()
    for _, k in ipairs({"AutoFarm", "PirateRaid", "AutoChest", "AutoCollectFruit", "FarmNearest", "FarmMob",
                        "FarmBoss", "EliteHunter", "PvpKill", "PvpHunt", "BoneFarm", "MasteryFarm", "AutoRaid"}) do
...
TabFarm:Slider("Tween Speed (lower = safer)", 80, 350, Cfg.TweenSpeed, function(v) Cfg.TweenSpeed = v end, 10)
...
TabVisual:Toggle("Rain Effect (GUI)", Cfg.Rain, function(v) Cfg.Rain = v end)
...
TabSet:Toggle("Rain Effect", Cfg.Rain, function(v) Cfg.Rain = v end)
...
print("[MorganHub] V3.1 loaded. Sea " .. Sea)
